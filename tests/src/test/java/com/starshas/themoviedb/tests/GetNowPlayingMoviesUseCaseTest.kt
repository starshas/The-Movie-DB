...
class GetNowPlayingMoviesUseCaseTest {
    ...
    private val dataMovieInstanceExample =
        DataMovieResponse.Movie(
            adult = false,
    ...
    private val domainMovieInstanceExample =
        DomainMoviesInfo.Movie(
            adult = false,
    ...
    private val dataDateRangeExample =
        DataMovieResponse.DatesRange(
            maximum = "2023-12-20T23:59:59-05:00",
            minimum = "2023-01-15T00:00:00-05:00",
        )

    private val domainDateRangeExample =
        DomainMoviesInfo.DateRange(
            maximum = "2023-12-20T23:59:59-05:00",
            minimum = "2023-01-15T00:00:00-05:00",
        )
    ...
    @Test
    fun `invoke returns success with MovieResponse`() =
        runTest {
            val dataMovieResponse =
                DataMovieResponse(
    ...
    @Test
    fun `invoke returns failure on generic error`() =
        runTest {
            ...
            val genericError: DomainApiError.GenericError =
                actualException as DomainApiError.GenericError
            assertEquals("Generic Exception", genericError.error.message)
        }

    @Test
    fun `invoke returns failure on HTTP error`() =
        runTest {
            val errorCode = 404
